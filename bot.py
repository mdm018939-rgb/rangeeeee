...
# ট্র্যাক রাখার জন্য range + message কম্বিনেশন মনে রাখা হবে (time বাদ, কারণ time বদলালেও
# একই কোড যেন আবার নতুন করে না পাঠায়)। প্রতিটা uid-এর সাথে শেষবার দেখা যাওয়ার সময়ও রাখা হচ্ছে,
# যাতে ১৫ মিনিট (API-এর window) পার হওয়া পুরনো এন্ট্রি মেমোরি থেকে ছেঁটে ফেলা যায়।
processed_cache = {}
CACHE_WINDOW = 16 * 60  # সেকেন্ড — API-এর ১৫ মিনিট উইন্ডোর চেয়ে একটু বেশি সেফটি মার্জিন
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if json_data.get("meta", {}).get("code") == 200:
                ...
                for hit in hits:
                    sms_range = hit.get("range")
                    message_text = hit.get("message")
                    if not sms_range:
                        continue

                    # শুধু range + message দিয়ে uid — time বাদ, কারণ একই কোড API-তে
                    # ভিন্ন ভিন্ন সময়ে বারবার ফিরে আসতে পারে (live feed, cached 5s ইত্যাদির কারণে)
                    uid = f"{sms_range}|{message_text}"

                    # যদি এই range-এ এই নির্দিষ্ট কোড আগে পাঠানো না হয়ে থাকে, তবেই এটা নতুন
                    if uid not in processed_cache and uid not in seen_this_batch:
                        new_messages_to_send.append((uid, hit))
                        seen_this_batch.add(uid)

                    # প্রতিটা hit দেখা মাত্রই তার শেষ-দেখা সময় আপডেট করুন (নতুন হোক বা পুরনো)
                    processed_cache[uid] = now

                # ১৬ মিনিটের বেশি পুরনো এন্ট্রি ছেঁটে ফেলুন — এগুলো API-এর ১৫ মিনিট উইন্ডোর
                # বাইরে চলে গেছে, তাই আর কখনো ফিরে আসবে না, মেমোরিতে জমিয়ে রাখার দরকার নেই
                processed_cache = {
                    k: v for k, v in processed_cache.items()
                    if now - v < CACHE_WINDOW
                }
                ...
                for uid, hit in reversed(new_messages_to_send):
                    ...
                    try:
                        ...
                        print(f"✅ নতুন চ্যানেলে পাঠানো হয়েছে! রেঞ্জ: {sms_range}")
                    except telebot.apihelper.ApiTelegramException as api_err:
                        if api_err.error_code == 429:
                            # Telegram নিজেই বলে দেয় কত সেকেন্ড পর retry করতে হবে
                            retry_after = api_err.result_json.get("parameters", {}).get("retry_after", 5)
                            print(f"⏳ Rate limited — {retry_after} সেকেন্ড অপেক্ষা করে আবার পাঠানো হচ্ছে...")
                            time.sleep(retry_after + 1)
                            try:
                                bot.send_message(CHAT_ID, tg_message, parse_mode="HTML", reply_markup=markup)
                                print(f"✅ retry-তে পাঠানো হয়েছে! রেঞ্জ: {sms_range}")
                            except Exception as retry_err:
                                print(f"⚠️ retry-তেও এরর: {retry_err}")
                        else:
                            print(f"⚠️ মেসেজ সেন্ড এরর: {api_err}")
