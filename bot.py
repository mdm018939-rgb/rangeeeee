        ...
        if response.status_code == 200:
            ...
            if json_data.get("meta", {}).get("code") == 200:
                ...
                for uid, hit in reversed(new_messages_to_send):
                    ...
                    # নতুন মেসেজ পাঠানো — 429 এলে Telegram এর বলা retry_after সময় মেনে আবার চেষ্টা করবে
                    max_retries = 3
                    for attempt in range(max_retries):
                        try:
                            ...
                            if send_err.error_code == 429:
                                retry_after = send_err.result_json.get("parameters", {}).get("retry_after", 5)
                                print(f"⏳ 429 পাওয়া গেছে, {retry_after} সেকেন্ড অপেক্ষা করছি... (attempt {attempt+1})")
                    ...
                    # প্রতিটা মেসেজের মাঝে ২ সেকেন্ড সেফটি বিরতি
                    time.sleep(1.5)
