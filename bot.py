...
# সব পাঠানো মেসেজ ১৫ মিনিট (AUTO_DELETE_SECONDS) পর ডিলিট করা হয়।
# প্রতিটা মেসেজের নিজস্ব delete_at সময় থাকে, তাই একসাথে সব মুছে যায় না।
# ══════════════════════════════════════════════════════════
AUTO_DELETE_SECONDS = 900  # ১৫ মিনিট
...
def schedule_delete(sender_bot, chat_id, message_id):
    """একটা মেসেজকে ১৫ মিনিট পর ডিলিট হওয়ার জন্য কিউতে যোগ করে"""
            ...
            if json_data.get("meta", {}).get("code") == 200:
                ...
                for i, (uid, hit) in enumerate(reversed(new_messages_to_send)):
                    ...
                    button3 = telebot.types.InlineKeyboardButton(
                        text="Copy Range",
                        copy_text=telebot.types.CopyTextButton(text=mask_range(str(sms_range))),
                        style=color3
                    )
                    markup.row(button3)
                    ...
                    try:
                        sent_msg = active_bot.send_message(CHAT_ID, tg_message, parse_mode="HTML", reply_markup=markup)
                        print(f"✅ (Bot {bot_label}) নতুন ওটিপি চ্যানেলে পাঠানো হয়েছে! রেঞ্জ: {sms_range}")

                        # 🗑️ ১৫ মিনিট পর অটো-ডিলিট হওয়ার জন্য শিডিউল করা হচ্ছে
                        schedule_delete(active_bot, CHAT_ID, sent_msg.message_id)
